...
enum StartFlowAction : uint8_t {
  START_FLOW_CONTINUE = 0,
  START_FLOW_CANCEL = 1,
  START_FLOW_RESTART = 2
};

static volatile bool startCancelRequested = false;
static volatile bool startRestartRequested = false;
...
volatile uint8_t activeSlaveMask = 3; // bit0=slave1, bit1=slave2
...
bool writeRegsMask(uint8_t mask, uint16_t addr, uint16_t *valueArr,
                   uint16_t count) {
  bool wrote = false;
  bool ok = true;
  if (mask & 1) {
...
void sendSigMask(uint8_t mask, uint16_t sig) {
  if (mask & 1)
    writeReg(SLAVE1_ID, HR_MASTER_SIG, sig);
  if (mask & 2)
    writeReg(SLAVE2_ID, HR_MASTER_SIG, sig);
}
...
uint8_t normalizedActiveMask() {
  uint8_t mask = activeSlaveMask & 0x03;
  return mask ? mask : 1;
}

bool modeUsesActiveMask() {
  return currentMode == 3 || currentMode == 4 || currentMode == 5;
}

void writeActive(uint16_t addr, uint16_t value) {
  uint8_t mask = normalizedActiveMask();
  if (mask & 1)
    writeReg(SLAVE1_ID, addr, value);
  if (mask & 2)
    writeReg(SLAVE2_ID, addr, value);
}

void idleInactiveSlaves() {
  uint8_t mask = normalizedActiveMask();
  uint16_t regs[4];
  regs[0] = 0;
  regs[1] = 0;
...
void pushModeAndColors() {
  ...
  if (modeUsesActiveMask())
    writeActive(HR_MODE, currentMode);
  else
...
void setMode(uint8_t m) {
  ...
  if (currentMode == 6)
    writeRegs(SLAVE1_ID, HR_SEC, regs, 4);
  else if (modeUsesActiveMask()) {
    writeRegsMask(normalizedActiveMask(), HR_SEC, regs, 4);
    idleInactiveSlaves();
  } else
...
void armCurrent() {
  ...
  if (currentMode == 3 || currentMode == 4)
    writeActive(HR_TARGET, targetSec);
...
StartFlowAction checkStartFlowControl() {
  if (startCancelRequested) {
    startCancelRequested = false;
    startRestartRequested = false;
    i2s_zero_dma_buffer(I2S_PORT);
    return START_FLOW_CANCEL;
  }
  if (startRestartRequested) {
    startRestartRequested = false;
    i2s_zero_dma_buffer(I2S_PORT);
    return START_FLOW_RESTART;
  }
  return START_FLOW_CONTINUE;
}

StartFlowAction playIntroWav(const char *path) {
  playWav(path);
  return checkStartFlowControl();
}

StartFlowAction waitIntroControlled(uint32_t ms, uint8_t mode) {
  uint32_t start = millis();
  while (millis() - start < ms) {
    StartFlowAction action = checkStartFlowControl();
    if (action != START_FLOW_CONTINUE)
      return action;
    if (mode == 1 && checkFalseStartMode1())
      return START_FLOW_CANCEL;
    if (mode == 2 && checkFalseStartMode2())
...
StartFlowAction startCurrent() {
  if (currentMode == 1) {
    // --- Hieu ung am thanh intro ---
    // teng -> nghi 1s -> onyourmask -> nghi 3s -> ready (dong thoi bao slave
    // dem xuong)
    StartFlowAction action = playIntroWav("/teng.wav");
    if (action != START_FLOW_CONTINUE)
      return action;
    if (checkFalseStartMode1())
      return START_FLOW_CANCEL;
    action = waitIntroControlled(1000, 1);
    if (action != START_FLOW_CONTINUE)
      return action;
    action = playIntroWav("/onyourmask.wav");
    if (action != START_FLOW_CONTINUE)
      return action;
    if (checkFalseStartMode1())
      return START_FLOW_CANCEL;
    action = waitIntroControlled(3000, 1);
    if (action != START_FLOW_CONTINUE)
      return action;
    if (checkFalseStartMode1())
      return START_FLOW_CANCEL;
  ...
  if (currentMode == 2) {
    ...
    if (checkFalseStartMode2())
      return START_FLOW_CANCEL;
    action = waitIntroControlled(1000, 2);
    if (action != START_FLOW_CONTINUE)
      return action;
    action = playIntroWav("/onyourmask.wav");
    if (action != START_FLOW_CONTINUE)
    ...
    if (checkFalseStartMode2())
      return START_FLOW_CANCEL;
    action = waitIntroControlled(3000, 2);
    if (action != START_FLOW_CONTINUE)
      return action;
    action = playIntroWav("/set.wav");
    if (action != START_FLOW_CONTINUE)
      return action;
    if (checkFalseStartMode2())
      return START_FLOW_CANCEL;
    action = waitIntroControlled(5000, 2);
    if (action != START_FLOW_CONTINUE)
      return action;
    if (checkFalseStartMode2())
      return START_FLOW_CANCEL;
  ...
  if (currentMode == 3 || currentMode == 4) {
    ...
    if (mask == 0x03)
      ...
      writeRegsMask(mask, HR_SEC, regs, 4);
  ...
  if (currentMode == 6) {
    ...
    if (action != START_FLOW_CONTINUE)
      return action;
    action = waitIntroControlled(1000, 6);
    if (action != START_FLOW_CONTINUE)
      return action;
    action = playIntroWav("/onyourmask.wav");
    if (action != START_FLOW_CONTINUE)
      return action;
    action = waitIntroControlled(3000, 6);
    if (action != START_FLOW_CONTINUE)
      return action;
...
void pauseCurrent() {
  ...
  if (currentMode == 2)
    ...
    sendSigMask(normalizedActiveMask(), SIG_PAUSE);
  else
...
void resumeCurrent() {
  ...
  if (currentMode == 2)
    ...
    sendSigMask(normalizedActiveMask(), SIG_RESUME);
  else
...
void tickMode34() {
  ...
  if (mask & 1) {
    ...
    s1Done = (!link1.online) || (ok1 && st1 == 200);
  }
  if (mask & 2) {
    bool ok2 = readReg(SLAVE2_ID, HR_COUNT_STATE, st2);
    s2Done = (!link2.online) || (ok2 && st2 == 200);
  }
  if (s1Done && s2Done) {
    mState = M_FINISHED;
  }
...
void startTaskFn(void * /*param*/) {
  while (true) {
    StartFlowAction result = startCurrent();
    if (result != START_FLOW_RESTART)
      break;
  }
  startCancelRequested = false;
  startRestartRequested = false;
...
void requestStartCurrent() {
  if (startTaskRunning) {
    startCancelRequested = false;
    startRestartRequested = true;
    return;
  }
  ...
  if (ok != pdPASS) {
    startTaskHandle = nullptr;
    while (startCurrent() == START_FLOW_RESTART) {
      delay(1);
    }
    startTaskRunning = false;
    startCancelRequested = false;
...
void apiStatus() {
  ...
  doc["activeSlaveMask"] = (uint8_t)normalizedActiveMask();
...
void apiPause() {
  ...
  startRestartRequested = false;
  if (startTaskRunning && mState != M_RUNNING)
    resetAll();
  else
    pauseCurrent();
  apiOk();
...
void apiSlaveMask() {
  ...
  int mask = server.arg("mask").toInt() & 0x03;
  if (mask == 0)
    mask = 1;
...
void apiSet() {
  ...
  if (n > 9999)
    n = 9999;
  uint8_t id = (uint8_t)sl;
  writeReg(id, HR_TARGET, (uint16_t)n);
  writeReg(id, HR_SYNC_FLAG, (uint16_t)(millis() & 0xFFFF));
  apiOk();
}
...
void apiDelCsv() {
  ...
  if (LittleFS.exists("/ket_qua_luot2.csv"))
    LittleFS.remove("/ket_qua_luot2.csv");
